...
class RepresentationPower(Scene):
    ...
    def construct(self):
        ...
        def move_intersect(t_intersect):
            ...
            ys_red = [coord(_t, 2 - 4 * (_t + 5) / 10 + r - 0.5) for _t, r in zip([t_1, t_2], ys_red_rand)]
            ys_blue = [coord(_t, -2 + 4 * (_t + 5) / 10 + r - 0.5) for _t, r in zip([t_1, t_2], ys_blue_rand)]

            red_path = VMobject(color=RED)
            red_path.set_points_smoothly([red_start.get_center(), ys_red[0], coord(t_intersect, 0), ys_red[1], red_end.get_center()])
            blue_path = VMobject(color=BLUE)
            blue_path.set_points_smoothly([blue_start.get_center(), ys_blue[0], coord(t_intersect, 0), ys_blue[1], blue_end.get_center()])
